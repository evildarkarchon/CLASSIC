...
public class GameIntegrityService(ConfigurationService config, LoggingService logger, GameVariables gameVars)
{
    public string CheckGameIntegrity()
    {
        ...
        var steamIniLocal =
            config.GetSetting<string>(YamlStore.GameLocal, $"Game{gameVars.Vr}_Info.Game_File_SteamINI");
        ...
        if (!string.IsNullOrEmpty(gameExeLocal) && File.Exists(gameExeLocal))
        {
            ...
            {
                messageBuilder.AppendLine(
                    $"\U0001F480 CAUTION : YOUR {rootName} GAME / EXE VERSION IS OUT OF DATE");
                messageBuilder.AppendLine("-----");
            }
            else
            {
            ...
            {
                messageBuilder.AppendLine(
                    $"✔️ Your {rootName} game files are installed outside of the Program Files folder!");
                messageBuilder.AppendLine("-----");
            }
            else
            {
    ...
    public string CheckXseIntegrity()
    {
        ...
        var adlibFileStr =
            config.GetSetting<string>(YamlStore.GameLocal, $"Game{gameVars.Vr}_Info.Game_File_AddressLib");

        if (string.IsNullOrEmpty(xseAcronym) || string.IsNullOrEmpty(xseFullName) ||
            string.IsNullOrEmpty(xseVerLatest))
        {
        ...
        {
            messageBuilder.AppendLine(
                $"❌ Value for Address Library is invalid or missing from CLASSIC {gameVars.Game} Local.yaml!");
            messageBuilder.AppendLine("-----");
        }
        ...
        {
            messageBuilder.AppendLine(
                $"❌ CAUTION : *{xseAcronym.ToLower()}.log* FILE IS MISSING FROM YOUR DOCUMENTS FOLDER!");
            messageBuilder.AppendLine(
                $"   You need to run the game at least once with {xseAcronym.ToLower()}_loader.exe");
    ...
    public string CheckXseHashes()
    {
        ...
        var messageBuilder = new StringBuilder();
        var xseScriptMissing = false;
        var xseScriptMismatch = false;
        ...
        foreach (var key in xseHashedScripts.Keys)
        {
            if (xseHashedScriptsLocal.TryGetValue(key, out var localHash))
            {
                ...
                {
                    messageBuilder.AppendLine(
                        $"[!] CAUTION : {key} Script Extender file is outdated or overridden by another mod!");
                    messageBuilder.AppendLine("-----");
